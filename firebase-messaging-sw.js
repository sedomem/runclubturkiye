...
// ── INSTALL — Service Worker kur ────────────────────────────────
self.addEventListener('install', function(event) {
  console.log('[SW] Install:', CACHE_VERSION);
  // NOT: cache.addAll() kaldırıldı. Statik dosyaların cache'lenmesi sırasında
  // oluşan hatalar (CORS, 404, vb.) tüm SW kurulumunu bloke ediyordu.
  // Fetch dinleyicisi dinamik olarak cache'leyecek.
  event.waitUntil(
    self.skipWaiting()
  );
});
